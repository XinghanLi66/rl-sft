...
def main():
    ...
    checkpoint_2 = Path('/homes/gws/lxh22/rl-sft/one-shot-em/checkpoints/Qwen2.5-Math-1.5B/one_shot_1.5b_t0.5')

    steps = [5, 10, 15, 20]
    steps_2 = [5, 10, 15, 20, 25, 30, 35, 40, 45, 50]
    all_scores = []
    
    for step in steps:
        print(f"Processing step: {step}")
        step_str = f'step_{step}'
        current_score = {'checkpoint': f'largerstep_{step * 4}'}
    ...
    for step in steps_2:
        ...
        for dataset in datasets:
            extracted = extract_scores_from_path(checkpoint_2 / step_str / f'temp00/{dataset}')
            if extracted: 
                current_score[dataset] = extracted

        for dataset in datasets_06:
            extracted = extract_scores_from_path(checkpoint_2 / step_str / f'temp06/{dataset}')
            if extracted: 
                current_score[dataset + '-t0.6'] = extracted
